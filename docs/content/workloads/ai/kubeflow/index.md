...
The route is generated by CDK8s — see the `kubeflow-dashboard` HTTPRoute in
{{ src(path="workloads/ai/kubeflow.go") }}.
    ...
    spec:
      ...
      nodeSelector:
        nvidia.com/gpu.present: "true"  # Routes to k8s-worker4 (the GPU node).
                                        # This GFD label is the selector; the taint
                                        # below is what makes the pod schedulable.
      tolerations:
      - key: dedicated                  # REQUIRED — worker4 carries
        operator: Equal                 # dedicated=ai:NoSchedule
        value: ai
        effect: NoSchedule
      containers:
      - resources:
