...
### 2026-02-24 17:00 UTC — Fix ArgoCD App Failures: nginx subchart, latest tags, version pins

**Problem**:
- Static analysis of all CDK8s app definitions revealed 4 code-level failures:
  1. **Infisical: stale nginx resources** — `infisical-standalone` chart v1.7.2 bundles an nginx-ingress subchart that renders a Deployment + `ValidatingWebhookConfiguration` even when `ingress.enabled: false`. The webhook can block all pod creation cluster-wide if the nginx pod is unhealthy.
  2. **Infisical: PostgreSQL `latest` tag** — `image.tag: latest` overridden in `infisical.go` risks a silent major-version upgrade on restart, which corrupts the PostgreSQL data directory.
  3. **N8N: `latest` image tag** — floating tag violates explicit-versioning policy; silently changed between restarts.
  4. **Headlamp: no Helm chart version pinned** — CDK8s Helm construct floats to the latest available chart, violating the pinning policy.
- Additionally, 5 app directories (`grafana`, `harbor`, `n8n`, `headlamp`, `rancher`) were absent from `/app/` because `cdk8s synth` requires `INFISICAL_*` env vars and panics early in CI when they are missing.

**Solution**:
1. **`infisical.go`**: Added `"ingress-nginx": {"enabled": false}` to Helm values to suppress the nginx subchart entirely and purge the `ValidatingWebhookConfiguration` from next synth.
2. **`infisical.go`**: Removed the `image.tag: latest` override from PostgreSQL values — Bitnami chart `16.2.5` ships PostgreSQL 16 by default.
3. **`n8n.go`**: Pinned n8n image to `1.78.0`; also bumped the n8n Helm chart from `1.16.28` → `1.16.29` (released 2026-02-20).
4. **`headlamp.go`**: Added `Version: "0.40.0"` (Headlamp chart released 2026-02-05).
5. The 5 missing app directories will be regenerated on next `just synth` run (requires `INFISICAL_*` env vars).

**Files Modified**:
- `platform/cdk8s/cots/seccomp/infisical.go` — nginx subchart disabled, PostgreSQL `latest` tag removed
- `platform/cdk8s/cots/automation/n8n.go` — image tag pinned to `1.78.0`, chart bumped to `1.16.29`
- `platform/cdk8s/cots/management/headlamp.go` — Helm chart version pinned to `0.40.0`

**Future Work — Bitnami PostgreSQL**:
The standalone Infisical chart uses Bitnami PostgreSQL as a bundled subchart. Bitnami licenses restrict commercial use above $5M ARR (homelab: unaffected). A future improvement would be migrating to **CloudNativePG (cnpg-io)** — a CNCF sandbox operator with no licensing caveats — but this requires a larger refactor involving `Cluster` CRDs and isn't blocking for homelab use.

**Verification**:
```bash
# Go compiles cleanly after all changes
cd platform/cdk8s && go build ./...    # exit 0

# After running 'just synth' (requires INFISICAL_* env vars):
# Verify nginx resources purged from infisical manifests:
ls app/infisical/Validating* 2>/dev/null && echo "FAIL" || echo "PASS: no webhook"
grep -r "ingress-nginx" app/infisical/ && echo "FAIL" || echo "PASS: nginx gone"

# Verify no 'latest' tags remain in source:
grep -r '"latest"' platform/cdk8s/cots/ && echo "FAIL" || echo "PASS: all pinned"

# Verify missing app dirs regenerated:
for app in grafana harbor n8n headlamp rancher; do
  [ -d "app/$app" ] && echo "PASS: $app" || echo "FAIL: $app missing"
done
```
...
### 2026-02-26 — GPU Stack, Observability, and Platform App Reorganization

#### NVIDIA GPU Stack Fixed on Talos

The NVIDIA GPU operator (v25.10.1) is now fully functional on Talos Linux with the RTX 5070 Ti on k8s-worker4.

**Root cause of previous failures**: GPU operator v25.x defaults to CDI (Container Device Interface) mode. CDI generates specs with `hostPath: /usr/lib/libEGL_nvidia.so.x`, but on Talos the NVIDIA libraries live at `/usr/local/glibc/usr/lib/`. CDI bind mounts silently failed, leaving GPU containers with no libraries.

**Fix**: `DEVICE_LIST_STRATEGY=envvar` (singular — not `envvars`). With envvar mode, the device plugin injects `NVIDIA_VISIBLE_DEVICES=<uuid>` into the container environment. The Talos `nvidia-container-runtime` (from the `nvidia-container-toolkit-production` extension) intercepts this env var and handles GPU injection using its own Talos-aware library paths.

**Additional fix — Validation Bridge DaemonSet**: GPU operator v25.x requires marker files at `/run/nvidia/validations/` to unblock device plugin startup. On Talos, the driver and toolkit DaemonSets have `DESIRED=0` (Talos extensions provide them), so these files are never created. A `busybox` DaemonSet now writes `.driver-ctr-ready`, `driver-ready`, and `toolkit-ready` to the hostPath on every GPU node, unblocking the device plugin init containers.

See `docs/nvidia-gpu-talos.md` for the complete journey.

**Key commits**:
- `01f03e2` — Switch GPU device plugin to envvars mode and add runtimeClassName to Ollama
- `5ee9915` — Correct DEVICE_LIST_STRATEGY to `envvar` (not `envvars`)

---

#### Ollama Running with GPU Access

Ollama (`ollama/ollama:0.17.1`) is running with full RTX 5070 Ti access, verified via `kubectl exec -n ollama deploy/ollama -- nvidia-smi`. The Ollama API is exposed externally at `http://ollama.madhan.app` via Gateway API HTTPRoute.

**Key commits**:
- `1fc0a52` — Expose Ollama API externally via Gateway API HTTPRoute

---

#### New Apps Added

| App | Folder | Namespace | Purpose |
|-----|--------|-----------|---------|
| ComfyUI | `ai/` | comfyui | Stable Diffusion / Flux image generation UI |
| Falco | `compliance/` | falco | Runtime security via eBPF syscall monitoring |
| OTel Collector | `monitoring/` | opentelemetry | Two-tier telemetry pipeline (agent + gateway) |

**Key commit**:
- `ac237b4` — Add ComfyUI, Falco, and OpenTelemetry Collector

---

#### Grafana Datasources Auto-Provisioned

VictoriaMetrics (Prometheus-compatible) and VictoriaLogs (Loki-compatible) datasources are now provisioned automatically via Grafana Helm values. No manual Grafana configuration is needed after deployment.

**Key commit**:
- `b6559ef` — Add VictoriaMetrics and VictoriaLogs datasources to Grafana

---

#### Package Restructure: seccomp/ → security/ + compliance/

The old `seccomp/` package was split into two purpose-driven packages:

| Old location | New location | Apps |
|---|---|---|
| `seccomp/infisical.go` | `security/infisical.go` | Infisical secrets platform |
| `seccomp/keyverno.go` | `compliance/keyverno.go` | Kyverno admission controller |
| `seccomp/trivy.go` | `compliance/trivy.go` | Trivy vulnerability scanning |
| — (new) | `compliance/falco.go` | Falco runtime security |

**Key commit**:
- `72f1221` — Rename seccomp/ → security/ + compliance/ packages
